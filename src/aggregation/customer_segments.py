...
from pyspark.sql.functions import (
    ...
    to_date, current_timestamp, explode, split, expr, ntile,
...
def load_customer_profiles():
    """Load customer profiles from Silver"""
...
def create_rfm_segmentation(customers, orders, process_date):
    ...
    try:
        ...
        # Add partitioning by a constant value to ensure data is properly distributed
        recency_window = Window.partitionBy(lit(1)).orderBy(col("recency"))
...
def create_product_affinity_segmentation(customers, order_items):
    ...
    try:
        ...
        # Add partitioning by customer_id to ensure data is properly distributed
...
def create_lifecycle_segmentation(customers, orders, process_date):
    ...
    try:
        ...
        # First check if the customer table already has a lifecycle_stage column
        has_lifecycle_column = "lifecycle_stage" in customers.columns
        if has_lifecycle_column:
            print("Found existing lifecycle_stage column in customers table - renaming it to avoid ambiguity")
            # Rename the column in the customers dataframe to avoid ambiguity
            customers = customers.withColumnRenamed("lifecycle_stage", "original_lifecycle_stage")
        ...
        # Use a consistent partitioning key for all window functions
        # Since we're applying segmentation rules across all customers, we can use a constant value
        # This ensures Spark won't need to shuffle all data to a single partition
        partition_key = lit(1)
        
        value_window = Window.partitionBy(partition_key).orderBy(col("total_spend").desc())
        recency_window = Window.partitionBy(partition_key).orderBy(col("days_since_last_order"))
        frequency_window = Window.partitionBy(partition_key).orderBy(col("order_count").desc())
        
        lifecycle_segments = lifecycle_metrics \
            .withColumn("value_rank", ntile(4).over(value_window)) \
            .withColumn("recency_rank", ntile(4).over(recency_window)) \
            .withColumn("frequency_rank", ntile(4).over(frequency_window)) \
            .withColumn("calculated_lifecycle_stage",  # Changed column name to avoid ambiguity
            ...
            .withColumn("customer_value_segment",
                when(col("value_rank") == 1, "Premium Value")
                .when(col("value_rank") == 2, "High Value")
                .when(col("value_rank") == 3, "Medium Value")
                .when(col("value_rank") == 4, "Low Value")
                .otherwise("Unknown")
            )
        
        # Rename the tenure days column to avoid ambiguity when joining
        lifecycle_segments = lifecycle_segments.withColumnRenamed("customer_tenure_days", "tenure_days")
        ...
        customer_lifecycle = customers \
            ...
            .select(
                ...
                col("tenure_days").alias("customer_tenure_days"),  # Rename back to original name
                col("calculated_lifecycle_stage").alias("lifecycle_stage"),  # Rename to standard column name
...
def create_kmeans_clusters(customer_data, num_clusters=5):
    """Create K-means clustering based on purchase behavior"""
    print("Creating K-means clustering")
    
    if customer_data is None:
        print("Missing customer data for K-means clustering")
        return None
    ...
    try:
        ...
        if missing_features:
            print(f"Missing required features for clustering: {missing_features}")
            # Reduce feature list to available columns
            features = [f for f in features if f in customer_data.columns]
            
        if not features:
            print("No valid features available for clustering")
            return None
        
        # Select features and handle null values
        cluster_data = customer_data.select(
            "customer_id", *features
        ).na.fill(0)  # Fill nulls with 0 for clustering
        ...
        kmeans = KMeans().setK(num_clusters).setSeed(1).setFeaturesCol("scaled_features")
        ...
        # Analyze clusters
        # Use a proper partitioning column for window operations
        cluster_analysis = clustered_data.groupBy("prediction").agg(
            count("customer_id").alias("cluster_size"),
            *[avg(f).alias(f + "_avg") for f in features]
        )
        
        # Create a window with proper partitioning
        cluster_window = Window.partitionBy(lit(1)).orderBy(col("cluster_size").desc())
        
        # Add ranking to clusters based on size
        cluster_analysis = cluster_analysis.withColumn("size_rank", row_number().over(cluster_window))
        
        # Create cluster labels based on characteristics
        cluster_analysis = cluster_analysis \
            .withColumn("cluster_label", 
                when(
                    (col("recency_avg").cast("double") < 30) & 
                    (col("frequency_avg").cast("double") > 5) & 
                    (col("monetary_avg").cast("double") > 100000), 
                    "High Value Active"
                )
                .when(
                    (col("recency_avg").cast("double") < 30) & 
                    (col("frequency_avg").cast("double") > 3), 
                    "Active Regular"
                )
                .when(
                    (col("recency_avg").cast("double") > 90) & 
                    (col("frequency_avg").cast("double") > 3), 
                    "Lapsed High Potential"
                )
                .when(
                    (col("frequency_avg").cast("double") <= 2), 
                    "New or Occasional"
                )
                .otherwise("Medium Value")
            )
        
        # Join cluster assignments back to the original data
        customer_clusters = customer_data.join(
            clustered_data.select("customer_id", "prediction"),
            "customer_id"
        )
        
        # Add cluster labels
        cluster_labels = cluster_analysis.select(
            col("prediction"), col("cluster_label")
        )
        
        customer_clusters = customer_clusters.join(
            cluster_labels,
            "prediction"
        ).withColumnRenamed("prediction", "cluster_id")
        
        print(f"Created {num_clusters} clusters for {customer_clusters.count()} customers")
        return customer_clusters
    except Exception as e:
        print(f"Error creating K-means clusters: {e}")
        return None
...
def create_customer_segments_gold(process_date):
    ...
    # Handle case where one segmentation might fail
    if rfm_segments is None:
        print("RFM segmentation failed, will continue with other segments")
    
    if product_segments is None:
        print("Product affinity segmentation failed, will continue with other segments")
    
    if lifecycle_segments is None:
        print("Lifecycle segmentation failed, will continue with other segments")
    
    # Check if we have at least one valid segmentation
    if rfm_segments is None and product_segments is None and lifecycle_segments is None:
        print("All segmentation methods failed, cannot proceed")
        return False
    
    # Use the first available segmentation to get customer IDs
    base_segments = None
    if rfm_segments is not None:
        base_segments = rfm_segments
    elif product_segments is not None:
        base_segments = product_segments
    else:
        base_segments = lifecycle_segments
    
    # Get list of customer IDs to ensure consistent joining
    customer_ids = base_segments.select("customer_id").distinct()
    
    # For missing segments, create empty dataframes with the customer IDs
    from pyspark.sql.types import StructType, StructField, StringType, IntegerType, DoubleType
    
    if rfm_segments is None:
        # Create schema for RFM segments
        rfm_schema = StructType([
            StructField("customer_id", StringType(), False),
            StructField("rfm_segment", StringType(), True)
        ])
        
        # Create empty dataframe with customer IDs
        rfm_segments = customer_ids.select(
            col("customer_id"),
            lit(None).cast(StringType()).alias("rfm_segment")
        )
    
    if product_segments is None:
        # Create empty product segments with customer IDs
        product_segments = customer_ids.select(
            col("customer_id"),
            lit(None).cast(StringType()).alias("primary_category"),
            lit(None).cast(StringType()).alias("secondary_category"),
            lit(None).cast(StringType()).alias("product_affinity_segment")
        )
    
    if lifecycle_segments is None:
        # Create empty lifecycle segments with customer IDs
        lifecycle_segments = customer_ids.select(
            col("customer_id"),
            lit(None).cast(StringType()).alias("lifecycle_stage"),
            lit(None).cast(StringType()).alias("purchase_frequency_segment"),
            lit(None).cast(StringType()).alias("customer_value_segment"),
            lit(None).cast(IntegerType()).alias("days_since_first_order"),
            lit(None).cast(IntegerType()).alias("days_since_last_order"),
            lit(None).cast(IntegerType()).alias("customer_tenure_days")
        )
    ...
    if combined_segments is not None:
        ...
        combined_segments.write \
            ...
            .save(MINIO_GOLD_CUSTOMER_SEGMENTS)
        ...
        if lifecycle_segments is not None:
            lifecycle_segments \
                ...
                .mode("overwrite") \
                .partitionBy("year", "month") \
                .save(MINIO_GOLD_LIFECYCLE_SEGMENTS)
...
if __name__ == "__main__":
    ...
    success = create_customer_segments_gold(process_date)

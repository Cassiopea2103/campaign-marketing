# Engagement metrics specific to social media
                ...
                # Conversion metrics
                conv_rate_min, conv_rate_max = self.platforms[platform]["conversion_rate_range"][ad_type]
    ...
    def generate_influencer_data(self, num_influencers=30):
        ...
        for i in range(num_influencers):
            # Select tier
            tier = random.choices(
                list(self.influencer_tiers.keys()), 
                weights=[0.6, 0.3, 0.1]
            )[0]
            
            # Basic profile
            followers_min, followers_max = self.influencer_tiers[tier]["followers_range"]
            followers = random.randint(followers_min, followers_max)
            
            fee_min, fee_max = self.influencer_tiers[tier]["fee_range"]
            base_fee = random.randint(fee_min, fee_max)
            
            engagement_min, engagement_max = self.influencer_tiers[tier]["engagement_range"]
            avg_engagement = random.uniform(engagement_min, engagement_max)
            
            # Platform and niche
            primary_platform = random.choice(self.influencer_platforms)
            niche = random.choice(self.influencer_niches)
        ...
        for i in range(num_influencers * 2):  # Each influencer does about 2 campaigns on average
            ...
            campaign_type = random.choice(["Lancement", "Promotion", "Branding"])
            campaign_name = f"{theme} - {campaign_type}"
            
            # Campaign dates
            campaign_start = fake.date_time_between(
                start_date=self.start_date,
                end_date=self.end_date - timedelta(days=30)
            )
            campaign_duration = random.randint(7, 30)  # 1 week to 1 month
            campaign_end = min(campaign_start + timedelta(days=campaign_duration), self.end_date)
            ...
            # Products featured
            products_featured = random.sample(self.highlighted_products, random.randint(1, 3))
            
            # Category
            category = random.choice(self.categories)
            
            # Promo code based on influencer name
            promo_code = f"{influencer['name'].split()[0].upper()}{random.randint(10, 30)}"
            
            # Performance metrics
            content_type = random.choice(contentimport pandas as pd
import numpy as np
import random
import json
import uuid
...
class AdvertisingDataGenerator:
    def __init__(self):
        """Initialize the advertising data generator"""
        # Date range for data generation
        self.start_date = datetime(2024, 1, 1)
        self.end_date = datetime(2025, 3, 15)  # Current date in the scenario
        
        # Ad platforms with their characteristics
        self.platforms = {
            "Facebook": {
        ...
        self.demographics = [
            ...
            {"age_min": 18,import pandas as pd
import numpy as np
import random
import datetime
from faker import Faker
